...
sensor.set_auto_exposure(False, -20)
...
while (kpts1 == None):
    img = sensor.snapshot()
    img.draw_string(0, 0, "Looking for a beetle...", color = 120)

    objects = img.find_features(beetle_cascade, threshold=0.5, scale=1.25)
    if objects:
        # Expand the ROI by 31 pixels in every direction
        beetle = (objects[0][0]-31, objects[0][1]-31,objects[0][2]+31*2, objects[0][3]+31*2)
        # Extract keypoints using the detect face size as the ROI
        kpts1 = img.find_keypoints(threshold=10, scale_factor=1.1, max_keypoints=100, roi=beetle)
        # Draw a rectangle around the first face
        img.draw_rectangle(objects[0], color = 25)
...
while (True):
    ...
    if (kpts2):
        ...
        if (match>5):
            ...
            print(kpts2, "matched:%d dt:%d"%(match, c[7]), color = 120)

    # Draw FPS
    img.draw_string(0, 0, "FPS:%.2f"%(clock.fps()), color = 120)
