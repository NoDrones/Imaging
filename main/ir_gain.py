...
def set_custom_exposure(high_mean_thresh = 54, low_mean_thresh = 53):
	try:
		...
		r_gain = round(r_gain/8)
		g_gain = round(g_gain/8)
		b_gain = round(b_gain/8)
		...
		while(((mean > high_mean_thresh) | (mean < low_mean_thresh)) & (count <= 256) & ((cur_gain >= 1.1) | (cur_gain <= 31.9))):
